 ...
 *           videoId:
 *             type: integer
 *             description: ID of video to comment on
 *             example: 1
 *           userId:
 *            type: integer
 *            description: ID of user who commented on the video
 *            example: 1
 *           content:
 *            type: string
 ...
 *  get:
 *    description: Use to request all comments by video id
 *    tags:
 *      - Comments
 *    parameters:
 *      - name: id
 *        in: path
 *        description: ID of video to fetch comments
 *        required: true
 *        type: integer
 *        minimum: 1
 *        example: 1
 ...
 *          description: Server error
 */
router.get('/video/:id', idParamValidator, async (req, res, next) => {
    try {
        let data;
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            ...
            data = await commentController.getCommentsByVideo(req.params.id);
            if (!data) {
                res.status(404).json({ result: 404, message: "Comment not found" });
            } else {
                res.send({ result: 200, data: data });
