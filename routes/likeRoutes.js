 ...
 * /api/likes/video/{id}:
 *  get:
 *    description: Use to request likes by video ID
 *    tags:
 *      - Likes
 *    parameters:
 *      - name: id
 *        in: path
 *        description: ID of video to fetch likes
 *        required: true
 *        type: integer
 *        minimum: 1
 *        example: 1
...
router.get('/video/:id', idParamValidator, async (req, res, next) => {
    try {
        ...
        if (!errors.isEmpty()) {
            ...
            const data = await likeController.getLikesByVideo(req.params.id);
            res.send({ result: 200, data: data });
        }
    } catch (err) {
        next(err);
 ...
 *            videoId:
 *              type: integer
 *              description: ID of video to like
 *              example: 1
 *            userId:
 *              type: integer
 *              description: ID of user who liked the video
 *              example: 1
 *    responses:
 *      '201':
